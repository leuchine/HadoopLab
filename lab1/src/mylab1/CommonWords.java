...
public class CommonWords {
	...
	public static class Mapper1 extends Mapper<Text, Text, Text, Text> {
		...
		public void map(Text key, Text value, Context context)
				throws IOException, InterruptedException {
			frequency.set(value.toString() + "_1");
	...
	public static class Mapper2 extends Mapper<Text, Text, Text, Text> {
		...
		public void map(Text key, Text value, Context context)
				throws IOException, InterruptedException {
			frequency.set(value.toString() + "_2");
			context.write(key, frequency);
		}
	...
	public static class CountCommonReducer extends
		...
		public void reduce(Text key, Iterable<Text> values,
			...
			int count1 = 0;
			int count2 = 0;
			String[] fre_i = new String[2];
			for(Text val: values) {
				fre_i = val.toString().split("_");
				if (fre_i[1].equals("1")) {
					count1 += Integer.parseInt(fre_i[0]);
				}		
				else if (fre_i[1].equals("2")) {
				    count2 += Integer.parseInt(fre_i[0]);	
				}
			}
